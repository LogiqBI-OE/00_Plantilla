...
const FIXED_LOGIN: Array<{ key: string; label: string; info: string }> = [
  { key: 'brand-hero-bg', label: 'Color de fondo', info: '--brand-hero-bg · Fondo del panel derecho del Login.' },
  { key: 'brand-hero-accent', label: 'Color de acento', info: '--brand-hero-accent · Chip MARCA·ALCANCE y decoraciones.' },
];

const FIXED_SIDEBAR: Array<{ key: string; label: string; info: string }> = [
  { key: 'sidebar-bg', label: 'Color de fondo', info: '--sidebar-bg' },
  { key: 'sidebar-active-text', label: 'Texto pagina activa', info: '--sidebar-active-text · El active-bg se deriva con alpha.' },
  { key: 'sidebar-section-title', label: 'Titulo de seccion', info: '--sidebar-section-title' },
  { key: 'sidebar-text', label: 'Texto de paginas', info: '--sidebar-text' },
  { key: 'sidebar-disabled-text', label: 'Texto deshabilitado', info: '--sidebar-disabled-text · Items "proximamente".' },
];

const THEMED: Array<{ key: string; label: string; info: string }> = [
  { key: 'bg-page', label: 'Fondo de pagina', info: '--bg-page' },
  { key: 'bg-card', label: 'Fondo de tarjetas', info: '--bg-card' },
  { key: 'border', label: 'Bordes', info: '--border' },
  { key: 'accent', label: 'Acento', info: '--accent · Botones primarios, links.' },
  { key: 'text-primary', label: 'Texto principal', info: '--text-primary' },
  { key: 'info', label: 'Info / exito', info: '--info' },
  { key: 'warning', label: 'Advertencia', info: '--warning' },
  { key: 'danger', label: 'Error', info: '--danger' },
];
...
export function PaletaSub({ brand, onSaved }: Props): React.ReactElement {
  ...
  const handleResetDefault = async () => {
    if (!confirm('Resetear paleta al default LogiQ?')) return;
    setSaving(true);
    try {
  ...
  const renderRow = (
    ...
    <div key={item.key} className="flex items-center gap-3 py-1.5 border-b border-border last:border-0">
      ...
      <TailwindColorPicker value={value} onChange={onChange} label={item.label} />
    </div>
  );
  ...
  return (
    <Card className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h3 className="font-semibold">Paleta</h3>
      ...
      <details open className="border border-border rounded-lg p-3" style={{ overflow: 'visible' }}>
        <summary className="cursor-pointer font-medium text-sm">Login (2 colores fijos)</summary>
        <div className="mt-2">
      ...
      <details open className="border border-border rounded-lg p-3" style={{ overflow: 'visible' }}>
        <summary className="cursor-pointer font-medium text-sm">Sidebar (5 colores fijos)</summary>
        <div className="mt-2">
      ...
      <div className="border border-border rounded-lg p-3">
        <div className="flex items-center justify-between mb-2">
          <span className="font-medium text-sm">Por tema (8 colores)</span>
          <div className="flex gap-1">
            {(['light', 'dark'] as const).map((th) => (
              <button
                ...
                type="button"
                onClick={() => setTheme(th)}
                className={`px-2.5 py-1 rounded-md text-xs font-medium ${theme === th ? 'bg-accent text-white' : 'opacity-60 hover:opacity-100'}`}
              >
      ...
      <div className="border border-border rounded-lg p-3 space-y-2">
        <div className="font-medium text-sm">Memorias ({brand.paletas_memoria.length}/5)</div>
